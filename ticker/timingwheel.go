...
import (
	"errors"
	"github.com/FelixHolmes/glc/ticker/delayqueue"
	"sync/atomic"
	"time"
	"unsafe"
)
...
// add inserts the ticker t into the current timing wheel.
...
// addOrRun inserts the ticker t into the current timing wheel, or run the
// ticker's task if it has already expired.
func (tw *TimingWheel) addOrRun(t *Timer) {
	if !tw.add(t) {
		// Already expired

		// Like the standard time.AfterFunc (https://golang.org/pkg/time/#AfterFunc),
		// always execute the ticker's task in its own goroutine.
		go t.task()
	}
}
...
//
// If there is any ticker's task being running in its own goroutine, Stop does
